...
def remove_percents(df, col):
    df.loc[df[col].notnull(), col] = pd.to_numeric(df.loc[df[col].notnull()][col].str.strip("%"))
    return df

def fill_zero_iron(df):
    df.loc[df["Iron (% DV)"].isnull(), "Iron (% DV)"] = 0
    return df
    
def fix_caffeine(df):
    df.loc[(df["Caffeine (mg)"]=="Varies") | (df["Caffeine (mg)"]=="varies"), "Caffeine (mg)"] = 0
    mean_avg = pd.to_numeric(df.loc[pd.to_numeric(df["Caffeine (mg)"])!=0, "Caffeine (mg)"]).mean()
    df.loc[pd.to_numeric(df["Caffeine (mg)"])==0, "Caffeine (mg)"] = mean_avg
    return df

def standardize_names(df):
    df["Beverage_prep"] = df["Beverage_prep"].str.replace(r"\(.*\) ?", "")
    df["Beverage_prep"] = df["Beverage_prep"].str.lower()
    return df
...
def main():
    ...
    # print(df.loc[(df["Caffeine (mg)"]!="Varies") & (df["Caffeine (mg)"]!="varies") ]["Caffeine (mg)"].to_numpy())
